        ...
        HStack(spacing: 12) {
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    // Child Selector
                    Menu {
                        ForEach(network.children, id: \.id) { child in
                            Button(action: {
                                network.selectedChild = child
                                // Refresh data for newly selected child
                                if let selectedChild = network.selectedChild {
                                    network.fetchHandoffSummary(childId: selectedChild.id)
                                    Task {
                                        try? await network.fetchMeals(childId: selectedChild.id)
                                    }
                                    network.fetchSleepLogs(childId: selectedChild.id)
                                    network.fetchActivityLogs(childId: selectedChild.id)
                                    network.fetchBehaviorLogs(childId: selectedChild.id)
                                    network.fetchHydrationLogs(childId: selectedChild.id)
                                    network.fetchActivityFeed(childId: selectedChild.id)
                                }
                            }) {
                                HStack {
                                    Text(child.name)
                                    if child.id == network.selectedChild?.id {
                        ...
                        HStack(spacing: 6) {
                            ...
                            Text(network.selectedChild?.name ?? "Select Child")
                                .font(.subheadline)
                                .foregroundColor(.secondary)
                                .lineLimit(1)
                                .truncationMode(.tail)
